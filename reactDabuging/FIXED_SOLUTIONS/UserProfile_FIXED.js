...
function UserProfile({ name = 'John', age = 30, email = 'john@example.com' }) {
  ...
  const selectUser = (userId) => {
    ...
    if (user) {
      ...
      setEditAge(user.age.toString()); // Convert to string for input
  ...
  // FIXED: Added proper data type validation and conversion
  const updateUser = () => {
    if (!editName.trim()) {
    ...
    // FIXED: Convert age string to number
  ...
  // FIXED: Properly handle user deletion and reset selection
  const deleteUser = (userId) => {
    if (users.length > 1) {
      const newUsers = users.filter(u => u.id !== userId);
      setUsers(newUsers);
      
      // FIXED: If deleting the currently selected user, select the first remaining user
